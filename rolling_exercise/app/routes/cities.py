...
async def get_best_cities(db: AsyncSession = Depends(get_db)) -> list[CityAqiAverageOut]:
    rows = await fetch_best_cities(db=db, limit=3)
...
async def get_city_measurements(city:str ,db: AsyncSession = Depends(get_db))-> list[AirQualityMeasurementOut]:
    measurments = await fetch_by_city(db=db,city=city)
    if not measurments:
        raise HTTPException(status_code=404,detail="unknown city")
    return measurments

@router.get("/{city}/average", response_model=CityAqiAverageOut)
async def get_average_city_aqi(city: str,db: AsyncSession = Depends(get_db),) -> CityAqiAverageOut:
    avg_aqi = await fetch_average_city_aqi(db=db, city=city)
